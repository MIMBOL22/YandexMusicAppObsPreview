...
let lastId; // Создаём ячейку в памяти, где мы будем хранить прошлую музыку

const yamapi = new YaMApi();
function update() { // Создаём функцию, которая обновляет TXT файл
    yamapi.updateSong().then(()=>{ // Спрашиваем приложение : "Какой трек сейчас играет?"
        const obj = yamapi.getSong(); // Получаем этот трек
...
setInterval(update, 1000); // Говорим, что обновление нужно выполнять каждую секунду (1000 миллисекунд)